package org.flixel
{
	...
	public class FlxCamera extends FlxBasic
	{
		 ...
		 * Camera "follow" style preset: camera has no deadzone, just tracks the focus object directly.
		 */
		static public const STYLE_LOCKON_VERTICAL:uint = 4;		
		...
		public function follow(Target:FlxObject, Style:uint=STYLE_LOCKON):void
		{
			...
			switch(Style)
			{
				...
				case STYLE_TOPDOWN_TIGHT:
					...
					deadzone = new FlxRect((width-helper)/2,(height-helper)/2,helper,helper);
					break;
				case STYLE_LOCKON_VERTICAL:
					helper = FlxU.max(width,height)/14;
					deadzone = new FlxRect(0,(height-helper)/2,FlxG.width ,helper);
					break;					
