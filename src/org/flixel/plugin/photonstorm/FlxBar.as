...
package org.flixel.plugin.photonstorm 
{
	...
	public class FlxBar extends FlxSprite
	{
		 ...
		 */
		public function FlxBar(x:int, y:int, direction:uint = FILL_LEFT_TO_RIGHT, width:int = 100, height:int = 10, parentRef:* = null, variable:String = "", min:Number = 0, max:Number = 100, border:Boolean = false, label:String = "", labelOffset:int = 60):void
		{
			super(x, y);
			...
			//if (label != "") {
				_label = new FlxText(x-labelOffset,y-3,91,label);
				_label.setFormat(null, 8, 0xffDBDBDB, "center");
				_label.scrollFactor.x = 0;
				_label.scrollFactor.y = 0
				_label.shadow = 0xff000000;
			...
			createFilledBar(0xffe1926c, 0xfffbe1e1, border);
		...
		public function trackParent(offsetX:int, offsetY:int):void
		{
			...
			if (parent.scrollFactor)
			{
				...
				_label.scrollFactor.x = parent.scrollFactor.x;
				_label.scrollFactor.y = parent.scrollFactor.y;				
